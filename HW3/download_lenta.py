...
class LentaParser:
    ...
    async def fetch(self, url: str, retries: int = 3):
        """Скачивает страницу с ретраями и экспоненциальным backoff.

        ВАЖНО: не бросает исключение наружу (чтобы не отменять другие таски),
        а возвращает None при окончательной неудаче.
        """
    ...
    async def _fetch_all_news_on_page(self, initial_html: str):
        ...
        # Уникализируем, сохраняя порядок (set() нельзя — ломает соответствие url→task)
